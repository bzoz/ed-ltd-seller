...
async function reacalculateBestTranser(db, system, commodity) {
  ...
  console.log(`BestTransfer ${system}:${commodity} - bestBuy ${bestBuyMarket} bestSell ${bestSellMarket}`)
  let bestPrice
...
async function getBestTransfers(limit) {
  const db = await getDB();
  const bestTransfers = await db.zrevrange('commodites-best-transfer', 0, limit)
