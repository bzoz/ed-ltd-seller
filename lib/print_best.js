...
function logStats() {
  ...
  process.stdout.write(`Got ${num(data.total)} messages: ${num(data.systems)} systems, ${num(data.stations)} staions, ${num(data.prices)} markets`)
}
...
async function printBestSell(commodity, limit) {
  ...
  if (result.length == 1)
    console.log(text('( no buying market found yet )'))
  else 
...
async function printBestBuy(commodity, limit) {
  ...
  if (result.length == 1)
    console.log(text('( no selling market found yet )'))
  else 
    console.log(table(result))
  for (let i = result.length; i < limit + 1; ++i)
    console.log('')
...
async function printBestTransfer(limit) {
  ...
  if (result.length == 1)
    console.log(text('( no in-system trade detected yet )'))
  else 
    console.log(table(result))
  for (let i = result.length; i < limit + 1; ++i)
    console.log('')
